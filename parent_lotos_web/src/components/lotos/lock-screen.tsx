    ...
    <main className="flex min-h-screen items-center justify-center bg-[#102b49] px-4 py-8">
      <section className="w-full max-w-md rounded-[32px] bg-white p-6 shadow-2xl sm:p-9">
        <div className="mb-10 flex items-center justify-between">
          ...
          <button onClick={() => auth.logout()} className="text-[#718097]">
            <LogOut size={20} />
          </button>
        </div>

        <h1 className="text-2xl font-black tracking-[-.06em]">С возвращением!</h1>
        <p className="mt-3 text-sm leading-6 text-[#718097]">
          Приветствуем, <strong className="text-[#19283c]">{auth.phone}</strong>.
          <br />
          Введите 4-значный PIN-код для разблокировки.
        </p>
        <input
          autoFocus
          value={auth.pin}
        ...
        >
          {auth.loading ? 'Разблокировка...' : 'Разблокировать'}
        </button>
      </section>
    </main>
  )
